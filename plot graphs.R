...
plot(mtcars)
pairs(mtcars[,1:3])
plot(women, type='p', pch=9,col='violet')# pch = plotting symbols
plot(women, type='l')
...
plot(y=women$height, x=women$weight, pch=15, xlab='Weight', ylab='Height', col='red', cex=2, type='b') #cex=amplification factor
...
text(x=1.2,y=c(58,62,65,68,72),c('min','Q1','Q2=median','Q3','max'))
?abline
...
abline(h=sum1)
text(x=1.5,y=sum1,c('min','Q1','Q2','Q3','max'),col='red')
#histogram
hist(women$height)
